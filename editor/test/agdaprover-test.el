...
(ert-deftest agdaprover-test-entry-command-binding-and-no-goal-selection ()
  (should (eq (lookup-key agdaprover-mode-map (kbd "C-c C-x C-t"))
              #'agdaprover-test-entries))
  (let (observed)
    (cl-letf (((symbol-function 'agdaprover--start-operation)
               (lambda (&rest arguments) (setq observed arguments))))
      (agdaprover-test-entries))
    (should (equal observed (list 'test-entries agdaprover-ranker)))))

(ert-deftest agdaprover-test-entry-command-never-saves-modified-source ()
  (with-temp-buffer
    (setq buffer-file-name "/tmp/NotWritten.agda")
    (insert "edited")
    (cl-letf (((symbol-function 'save-buffer) (lambda (&rest _) (ert-fail "Must not save")))
              ((symbol-function 'agdaprover--goal-at-point) (lambda () (ert-fail "Must not select a goal"))))
      (should-error (agdaprover-test-entries) :type 'user-error))
    (should (buffer-modified-p))
    (should (equal (buffer-string) "edited"))))

(ert-deftest agdaprover-test-entry-report-handles-split-lines-and-stale-runs ()
  (let ((source (generate-new-buffer " *entry-source*"))
        (report (generate-new-buffer " *entry-report*"))
        (process (make-pipe-process :name "entry-stream" :noquery t))
        stopped)
    (unwind-protect
        (progn
          (with-current-buffer source (setq agdaprover--process process))
          (with-current-buffer report (setq agdaprover--entry-report-process process))
          (process-put process 'agdaprover-source-buffer source)
          (process-put process 'agdaprover-source-tick
                       (with-current-buffer source (buffer-chars-modified-tick)))
          (process-put process 'agdaprover-entry-report report)
          (process-put process 'agdaprover-request-id "test:entries")
          (process-put process 'agdaprover-source-sha256 "abc")
          (let* ((value '((schema_version . "agdaprover.editor.event.v1")
                          (request_id . "test:entries") (operation . "test-entries")
                          (source_sha256 . "abc") (event . "entry-result")
                          (payload . ((status . "verified") (solution . "keep x = x")
                                      (elapsed_ms . 12)
                                      (validation . ((fresh_process . t) (checked . t)))))))
                 (line (concat (json-serialize value) "\n")))
            (agdaprover--entry-test-filter process (substring line 0 17))
            (should (= 0 (with-current-buffer report (buffer-size))))
            (agdaprover--entry-test-filter process (substring line 17))
            (should (string-match-p "keep x = x" (with-current-buffer report (buffer-string))))
            (should-not (buffer-local-value 'agdaprover--last-result source))
            (with-current-buffer source (insert "changed"))
            (cl-letf (((symbol-function 'agdaprover--stop-process)
                       (lambda (candidate) (setq stopped candidate))))
              (agdaprover--entry-test-filter process line))
            (should (eq stopped process))
            (should (process-get process 'agdaprover-entry-error))))
      (delete-process process)
      (kill-buffer source)
      (kill-buffer report))))

(ert-deftest agdaprover-test-real-independent-entry-tests-never-apply-proofs ()
  (skip-unless (agdaprover-test--supported-agda-p))
  (skip-unless (file-executable-p (expand-file-name "native/source-parser/target/agdaprover-source-parser"
                                                   agdaprover-test--root)))
  (let* ((directory (make-temp-file "agdaprover-entry-test-" t))
         (file (expand-file-name "EntryTest.agda" directory))
         (text "module EntryTest where\nid : {A : Set} → A → A\nid x = x\nfirst : {A : Set} → A → A → A\nfirst x y = x\n")
         (agdaprover-project-root agdaprover-test--root)
         (agdaprover-apply-policy 'always)
         (original-special-binding (lookup-key special-mode-map (kbd "C-c C-x C-k")))
         source report)
    (unwind-protect
        (progn
          ...
          (with-current-buffer source
            (let ((tick (buffer-chars-modified-tick)))
              (cl-letf (((symbol-function 'agdaprover--apply-result)
                         (lambda (&rest _) (ert-fail "Entry tests must not apply proofs"))))
                (agdaprover-test-entries)
                (setq report agdaprover--last-output-buffer)
                (should (agdaprover-test--wait-until (lambda () (null agdaprover--process)) 30)))
              (should (equal original-special-binding
                             (lookup-key special-mode-map (kbd "C-c C-x C-k"))))
              (should (= tick (buffer-chars-modified-tick)))
              (should-not (buffer-modified-p))
              (should-not agdaprover--last-result)
              (should (equal (buffer-string) text))))
          (should (string-match-p "Finished: 2/2 entries solved" (with-current-buffer report (buffer-string))))
...
(ert-deftest agdaprover-test-reload-preserves-enabled-buffers-and-settings ()
  (let ((buffers (list (generate-new-buffer " *agdaprover-reload-first*")
                       (generate-new-buffer " *agdaprover-reload-second*")))
        (agdaprover-python-command "custom-python")
        (agdaprover-project-root "/unused-backend-checkout/")
        (agdaprover-ranker 'symbolic)
        (agdaprover-mode-hook nil)
        (hook-calls 0)
        (mode-map agdaprover-mode-map)
        (original-command (symbol-function 'agdaprover-reserved-n)))
    (unwind-protect
        (progn
          (add-hook 'agdaprover-mode-hook (lambda () (cl-incf hook-calls)))
          (dolist (buffer buffers)
            (with-current-buffer buffer
              ;; Exercise the minor mode without starting Agda's own process.
              (setq major-mode 'agda2-mode)
              (insert "example = {!!}\n")
              (goto-char 4)
              (setq-local agdaprover-agda-executable "custom-agda")
              (setq agdaprover--last-result '((status . "verified")))
              (setq agdaprover--last-source-tick (buffer-chars-modified-tick))
              (agdaprover-mode 1)))
          (let ((obsolete-command (lambda () (error "Obsolete definition"))))
            (fset 'agdaprover-reserved-n obsolete-command)
            (with-current-buffer (car buffers)
              (agdaprover-reload)
              (should (eq (current-buffer) (car buffers)))
              (should-not (eq (symbol-function 'agdaprover-reserved-n)
                              obsolete-command))
              (agdaprover-reload)))
          (should (= hook-calls 2))
          (should (eq mode-map agdaprover-mode-map))
          (should (= 1 (cl-count 'agdaprover-mode minor-mode-map-alist :key #'car)))
          (should (equal agdaprover-python-command "custom-python"))
          (should (equal agdaprover-project-root "/unused-backend-checkout/"))
          (should (eq agdaprover-ranker 'symbolic))
          (should (string-match-p "agdaprover-reload"
                                  (prin1-to-string agdaprover-mode-menu)))
          (dolist (buffer buffers)
            (with-current-buffer buffer
              (should agdaprover-mode)
              (should (eq (key-binding (kbd "C-c C-x C-q")) #'agdaprover-reload))
              (should (equal (buffer-string) "example = {!!}\n"))
              (should (buffer-modified-p))
              (should (= (point) 4))
              (should (= agdaprover--last-source-tick (buffer-chars-modified-tick)))
              (should (equal agdaprover-agda-executable "custom-agda"))
              (should (equal agdaprover--last-result '((status . "verified")))))))
      (fset 'agdaprover-reserved-n original-command)
      (mapc #'kill-buffer buffers))))

(ert-deftest agdaprover-test-reload-loads-source-not-compiled-code-or-backend ()
  (let ((agdaprover-project-root "/other-backend/")
        (expected (expand-file-name "editor/agdaprover.el"
                                    agdaprover--default-project-root))
        observed)
    (cl-letf (((symbol-function 'load)
               (lambda (&rest args) (setq observed args) t)))
      (agdaprover-reload))
    (should (equal observed (list expected nil t t)))))

(ert-deftest agdaprover-test-reload-rejects-missing-source ()
  (cl-letf (((symbol-function 'file-readable-p) (lambda (_path) nil))
            ((symbol-function 'load) (lambda (&rest _) (ert-fail "Must not load"))))
    (should-error (agdaprover-reload) :type 'user-error)))

(ert-deftest agdaprover-test-reload-does-not-report-success-on-load-failure ()
  (let (reported)
    (cl-letf (((symbol-function 'load) (lambda (&rest _) (error "Invalid update")))
              ((symbol-function 'message)
               (lambda (&rest args) (push args reported))))
      (should-error (agdaprover-reload)))
    (should-not reported)))

(ert-deftest agdaprover-test-reload-blocks-owned-runs-in-other-buffers ()
  (with-temp-buffer
    (let ((busy-buffer (current-buffer)))
      ;; Ownership, not process-live-p, also covers an exited child whose
      ;; sentinel has not yet delivered the result.  No child is needed here.
      (setq agdaprover--process 'pending-result)
      (with-temp-buffer
        (cl-letf (((symbol-function 'load)
                   (lambda (&rest _) (ert-fail "Must not reload during a run"))))
          (let ((failure (should-error (agdaprover-reload) :type 'user-error)))
            (should (string-match-p (regexp-quote (buffer-name busy-buffer))
                                    (error-message-string failure))))))
      (should (eq agdaprover--process 'pending-result)))))

(ert-deftest agdaprover-test-keybinding-refresh-preserves-customizations ()
  (let ((agdaprover-mode-map (make-sparse-keymap))
        (agdaprover--installed-keybindings nil))
    (agdaprover--install-keybindings
     '(("C-c C-x C-p" . old-proof)
       ("C-c C-x C-d" . old-deep)
       ("C-c C-x C-k" . old-cancel)
       ("C-c C-x C-v" . old-apply)))
    (define-key agdaprover-mode-map (kbd "C-c C-x C-d") #'ignore)
    (define-key agdaprover-mode-map (kbd "C-c C-x C-k") nil)
    (define-key agdaprover-mode-map (kbd "C-c C-x C-u") #'ignore)
    (agdaprover--install-keybindings
    ...
    (should (eq (lookup-key agdaprover-mode-map (kbd "C-c C-x C-p"))
                #'agdaprover-prove-goal))
    (should (eq (lookup-key agdaprover-mode-map (kbd "C-c C-x C-q"))
                #'agdaprover-reload))
    (should-not (lookup-key agdaprover-mode-map (kbd "C-c C-x C-v")))
    (should-not (lookup-key agdaprover-mode-map (kbd "C-c C-x C-k")))
    (should (eq (lookup-key agdaprover-mode-map (kbd "C-c C-x C-d")) #'ignore))
    (should (eq (lookup-key agdaprover-mode-map (kbd "C-c C-x C-u")) #'ignore))))

(ert-deftest agdaprover-test-keybinding-refresh-respects-custom-prefix ()
  (let ((agdaprover-mode-map (make-sparse-keymap))
        (agdaprover--installed-keybindings nil))
    (define-key agdaprover-mode-map (kbd "C-c C-x") #'ignore)
    (agdaprover--install-keybindings '(("C-c C-x C-q" . agdaprover-reload)))
    (should (eq (lookup-key agdaprover-mode-map (kbd "C-c C-x")) #'ignore))))
