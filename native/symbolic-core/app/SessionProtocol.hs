...
data Scheduling = Scheduling
  ...
  , schedulingStagedPlanning :: Bool, schedulingComputationDirected :: Bool }
...
defaultScheduling = Scheduling
  ...
  , schedulingStagedPlanning = True, schedulingComputationDirected = True }
...
instance FromJSON Scheduling where
  parseJSON = withObject "scheduling" $ \o -> do
    ...
    computationDirected <- switch "computation_directed_splits"
    ...
    pure Scheduling
      ...
      , schedulingStagedPlanning = staged, schedulingComputationDirected = computationDirected }
    ...
    resolved key $ \root -> do
      ...
      case ((++) <$> loaded <*> focusedModel) >>= Policy.models of
        ...
        Right models -> Run.start oneMove runs root G.Settings
          ...
          , G.stagedPlanning = schedulingStagedPlanning scheduling
          , G.computationDirectedSplits = schedulingComputationDirected scheduling } nativePath selection
